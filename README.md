...
**Текущая реализация.** XR Blocks depth mesh принимает лучи импульсов, а plane detection — тот же слой отладки. Симулятор даёт синтетическую комнату, устройство — доступную ему геометрию. Частицы, волны, четыре жеста, DEBUG/DREAM и безопасная fallback-комната работают без серверной части. Физический шаг сделан: усиленный импульс выпускает осколки-динамические тела (Rapier с CDN, пул ≤32, CCD), которые по-настоящему скачут по живому depth-mesh и рождают ударные кольца в точках контакта; если движок не загрузился, опыт честно сообщает `PHYSICS RAYCAST` и живёт на рейкастах.
...
**Текущая реализация.** Ветер, дождь, облачность, температура, давление и диапазон −24…+24 часа связаны с одной почасовой выборкой. Ливень — полосы-билборды, растянутые скоростью падения и сносом ветра; морось и снег — точки. На устройстве с depth-сенсором дождь осведомлён о реальной геометрии: капли гаснут за поверхностями и садятся точно на настоящие столешницы (пятно-декаль + кольцо), рождение новых капель привязано к объёму комнаты. Над головой — объёмное облако (в VR и на десктопе; телефонному AR остаётся шейдерная плита), по кнопке SOUND — звук дождя с громкостью по интенсивности осадков. Найденные плоскости ловят капли декелями, без плоскостей дождь честно падает на пол. HUD доступен на телефоне и десктопе, в XR тот же ряд вынесен в пространственное управление.
...
**Текущая реализация.** Есть настольный и 360° режимы, геолокация, выбор точки лучом, манипулируемая карточка, кнопка радиуса и масштаб двумя руками в XR. Размещение — как в каноне SDK: макет-призрак следует за попаданием луча в реальную поверхность, тап фиксирует его с посадочной анимацией, долгое удержание подложки — перестановка; two-source pinch теперь честно берёт кончики указательных пальцев (или контроллеры). В телефонном AR голограмма получает rim-подсветку и приподнятую яркость линий — тёмная масса перестаёт быть пятном. В выборку входят культурные места, памятники, кафе, образование и другие именованные POI; визуальный язык остаётся абстрактной орбитальной схемой.
...
**Текущая реализация.** Живой FFT строит световую архитектуру, классификация различает речь, устойчивый музыкальный спектр и ударный транзиент. FREEZE копит галерею: до пяти снимков стоят рядом, каждый можно перенести (drag тела), повернуть (drag кольца-основания), растворить удержанием ~0.7 с или squeeze; старейший вытесняется, тепло-тинт и статус показывают возраст. Без разрешения на микрофон опыт не пустует: демонстрационный генератор по очереди показывает все три состояния.
...
**Текущая реализация.** Кольцевой буфер хранит луч и импульсы, старые события исчезают через минуту, выбор ✦ реконструирует четыре цветных временных слоя. Слайдер −60…0 с перематывает сцену целиком: позы-призраки с интерполяцией, импульсы и следы в цвете возраста (теплое — свежее, холодное — старое), прошлое read-only; над выбранным импульсом висит карточка с точным возрастом. На Quest — луч руки/контроллера; на телефоне и десктопе — тот же select-путь XR Blocks.
...
- `?debug=1` — публикует `window.xb` и `window.xbReady` для диагностического harness, а у REALITY//FIELD и WEATHER//ROOM ещё и подсвечивает найденные плоскости комнаты;
- без параметров кнопка **Start Simulator** остаётся рядом с входом в XR.

В режиме simulator: WASD — движение, Q/E — высота, правая кнопка + мышь — взгляд, левая кнопка — select; Left Shift переключает режимы (`modeToggle` включён во всех опытах), причём опыт стартует в режиме User, чтобы клик мышью сразу работал как select, а позы рук для проверки жестов доступны переключением. В каждой `xrblocks/*/index.html` подключён `lit`, чтобы работали официальные панели настроек и поз рук.

Симулятор не эмулирует WebXR: depth, plane detection и позы рук в нём синтетические. Поэтому его экраны доказывают логику, состояния и композицию, но приёмка сенсоров и производительности остаётся за Quest 3.
