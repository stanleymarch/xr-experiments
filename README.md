...
- **[`experiments/`](experiments/)** — 8th Wall / A-Frame camera AR for phones, each with its own webpack config.
...
| WEATHER//ROOM | XR Blocks | Live outside weather rendered inside your room (Open-Meteo, one request); hand-scrubbed −24h…+24h timeline | planned |
| CITY//ORBIT | XR Blocks | Your street from OpenStreetMap as a tabletop hologram or 360° shell; pull POIs up into cards; scale the city between two hands | planned |
| SOUND//SPACE | XR Blocks | Microphone sound sculpted into 3D structures in real time; freeze phrases into glowing sculptures, walk through the history | planned |
| ECHO//ROOM | XR Blocks | Every action leaves a 60-second trail; tap an old trace to unfold NOW/−1s…−4s temporal layers around it | planned |
| [Knockdown](experiments/knockdown/) | 8th Wall | Knock physics towers off your table | playable |
| [Portal](experiments/portal/) | 8th Wall | Place a portal on a real wall, peek through | playable |
| [Sea Battle](experiments/sea-battle/) | 8th Wall | Classic naval battle laid out in your room | playable |
...
experiments/            8th Wall experiences (webpack, one folder each)
...
- **8th Wall:** create `experiments/<name>/` with `config/webpack.config.js` emitting `dist/` (see any existing one), then wire `build:<name>` / `serve:<name>` scripts in `package.json`.
