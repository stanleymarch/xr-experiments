...
| Официальные скиллы (методики) | `node_modules/xrblocks/skills/`: `xb-add-interactions`, `xb-add-spatial-ui`, `xb-add-world-sensing`, `xb-anchors` |
| Мануалы | <https://xrblocks.github.io/docs/manual/>: Interaction, Placement, UI, World, Depth, Simulator и др. |
| Репо с шаблонами/сэмплами | <https://github.com/google/xrblocks> (`templates/`, `samples/`, `demos/`) — **не входят в npm-пакет** |
...
## Долги, найденные при исследовании

- `hud.js` follow — переписать на `FollowHead`/`FaceCamera` или `UIOverlay`
  при следующей правке HUD (меньше своего кода, семантика из мануала).
- У всех опытов включён `enableReticles`, но `projectOnDepthMesh` не включён:
  на Quest ретикл не проецируется на реальные поверхности — включить в
  reality-field вместе с depth-настройками, если нужен «прицел по комнате».
